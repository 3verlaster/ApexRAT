	...
	try:
		...
		if obfuscation_var.get() == "on":
			if not uac_admin_var.get() == "on":
				...
				try:
					process = Popen(f'pyarmor pack -e" --onefile --noconsole --uac-admin --icon NONE" {file_to_compile}', creationflags=CREATE_NEW_CONSOLE)
				except Exception as e:
					print(e)
		else:
			if not uac_admin_var.get() == "on":
				...
				try:
					process = Popen(f'pyinstaller --onefile --noconsole --uac-admin {file_to_compile}', creationflags=CREATE_NEW_CONSOLE)
				except Exception as e:
					print(e)
...
uac_admin_var = ct.StringVar(value="off")
#####################
...
obfuscation_checkbox = ct.CTkCheckBox(checkboxes_frame, text="Admin privileges", variable=uac_admin_var, onvalue="on", offvalue="off").place(x=10, y=70)
compiling_label = ct.CTkLabel(root, text="Compiling in process...", font=compiling_font, text_color="red")
